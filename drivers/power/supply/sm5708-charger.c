...
#define REG_FLEDCNTL6_BOOST_VOLT	RANGE(mV(4000), mV(100), mV(5100))
...
/* Lookup tables for decoding register values */
static const int sm5708_otg_curr_ua[] = {
	mA(500), mA(700), mA(900), mA(1500),
};
...
struct sm5708_charger {
	...
	struct sm5708_vreg vreg_otg_vbus;
	struct sm5708_vreg vreg_flash_boost;
...
static int sm5708_switch_mode(struct sm5708_charger* charger)
{

	/* TODO */
	regmap_update_bits(charger->regmap, SM5708_REG_FLEDCNTL6,
			REG_FLEDCNTL6_BOOST_VOLT_MASK,
			FIELD_PREP(REG_FLEDCNTL6_BOOST_VOLT_MASK, 5));
	regmap_update_bits(charger->regmap, SM5708_REG_CHGCNTL5,
			REG_CHGCNTL5_OTG_CURR_MASK,
			FIELD_PREP(REG_CHGCNTL5_OTG_CURR_MASK, 0));
	regmap_update_bits(charger->regmap,
			SM5708_REG_CNTL,
			REG_CNTL_MODE_MASK,
			REG_CNTL_MODE_CHG_ON);
...
{
	struct sm5708_vreg *vreg = rdev_get_drvdata(rdev);

	vreg->enabled = true;

	return sm5708_switch_mode(vreg->charger);
}
...
static const struct regulator_desc sm5708_otg_vbus_rdesc = {
	.id = 0,
	.name = "otg-vbus",
	.of_match = "otg-vbus",
	.ops = &sm5708_vreg_ops,
...
static const struct regulator_desc sm5708_flash_boost_rdesc = {
	.id = 1,
	.name = "flash-boost",
			       ...
			       union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_CHARGE_BEHAVIOUR:
		val->intval = charger->charge_behaviour;
		break;
				...
				enum power_supply_property psp, const union power_supply_propval *val)
{
	...
	switch (psp) {
	case POWER_SUPPLY_PROP_CHARGE_BEHAVIOUR:
		charger->charge_behaviour = val->intval;
		gpiod_set_value(charger->charge_en_gpio,
				charger->charge_behaviour == POWER_SUPPLY_CHARGE_BEHAVIOUR_AUTO);
		break;
	case POWER_SUPPLY_PROP_CHARGE_TERM_CURRENT:
...
static int sm5708_charger_probe(struct platform_device *pdev)
{
	...
	config.dev = dev;
	config.driver_data = &charger->vreg_flash_boost;

	rdev = devm_regulator_register(dev, &sm5708_flash_boost_rdesc, &config);
	if (IS_ERR(rdev))
		return dev_err_probe(dev, PTR_ERR(rdev), "failed to register regulators");

	config.driver_data = &charger->vreg_otg_vbus;

	rdev = devm_regulator_register(dev, &sm5708_otg_vbus_rdesc, &config);
	if (IS_ERR(rdev))
		return dev_err_probe(dev, PTR_ERR(rdev), "failed to register regulators");
